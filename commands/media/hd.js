...
const API_HD_UPLOAD_URL = "https://dv-yer-api.online/image/hd/upload";
const API_HD_URL = "https://dv-yer-api.online/image/hd";
const API_KEY = "dvyer916005230981";
...
async function downloadQuotedImageBuffer(msg) {
  ...
  if (buffer.length > MAX_IMAGE_BYTES) throw new Error("La imagen es demasiado grande para procesarla.");
      ...
      if (!quotedImage && !directUrl) {
        ...
        return reply(
          ...
          `*📸 USO: .hd / .remini*\n\n` +
            ...
            `.remini (respondiendo una imagen)`,
          msg
        );
      }
